...
(defn- stone-adjacent-vertices
  [stone]
  (let [[column line] (m/stone-vertex stone)]
...
(defn- group-vertices
  [group]
  (set (map m/stone-vertex group)))

(defn- adjacent-to-stone?
  [stone group]
  (when (= (m/stone-color stone) (group-color group))
...
(defn- has-empty-adjacent-vertex?
  [stones stone]
  (let [stones-vertices (set (map m/stone-vertex stones))
